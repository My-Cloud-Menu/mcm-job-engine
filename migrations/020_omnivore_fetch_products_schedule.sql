-- Wire the Omnivore inventory/menu sync into the scheduler + manual trigger.
-- ensure_sync_schedules: always provision an omnivore 'fetch_products' row (24h),
-- active iff config.syncProductsAutomatically. trigger_sync_now: whitelist it.
...
declare
  ...
  v_tables_on boolean := false; v_products_on boolean := false; v_config jsonb;
begin
  ...
  return jsonb_build_object('affected', v_affected, 'action', 'synced',
    'orders', v_orders_on, 'payments', v_payments_on, 'push', v_push_on, 'tables', v_tables_on, 'products', v_products_on);
end;
$function$;
...
begin
  if not (
    ...
    or (p_integration = 'omnivore' and p_sync_type in ('fetch_recent_orders', 'fetch_tables', 'fetch_products'))
  ) then
...
insert into sync_schedules (site_id, integration, sync_type, interval_seconds, status, next_run_at)
select si.site_id, 'omnivore', 'fetch_products', 86400, 'disabled', now()
