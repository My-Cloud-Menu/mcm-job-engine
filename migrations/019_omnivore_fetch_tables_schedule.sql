-- Phase 4: wire the Omnivore floor-tables sync into the scheduler + manual trigger.
-- 1) ensure_sync_schedules: always provision an omnivore 'fetch_tables' row (24h),
--    active iff config.syncTablesAutomatically. The row always exists so that
--    trigger_sync_now always has a schedule_id for the manual "sync now".
-- 2) trigger_sync_now: whitelist omnivore 'fetch_tables'.
-- 3) Backfill a disabled 'fetch_tables' row for existing active omnivore sites.
...
begin
  ...
  return jsonb_build_object(
    ...
    'orders', v_orders_on, 'payments', v_payments_on, 'push', v_push_on, 'tables', v_tables_on
  );
end;
$function$;
...
begin
  if not (
    ...
    or (p_integration = 'omnivore' and p_sync_type in ('fetch_recent_orders', 'fetch_tables'))
  ) then
  ...
  v_job_id := enqueue_job(
    ...
    jsonb_build_array(jsonb_build_object(
      'step_name', p_sync_type, 'max_attempts', 3,
...
-- Backfill: ensure every active omnivore site has a (disabled) fetch_tables row so
-- the manual "sync now" always finds a schedule, even before the toggle is flipped.
insert into sync_schedules (site_id, integration, sync_type, interval_seconds, status, next_run_at)
select si.site_id, 'omnivore', 'fetch_tables', 86400, 'disabled', now()
from site_integrations si
